...
![Пять серых траекторий случайного процесса, идущих слева направо и расходящихся с ростом времени; вертикальная линия в момент t пересекает все траектории, а рядом с ней отложена плотность распределения значений в этом сечении, показывающая разброс по ансамблю](../../../svg/probability/process-ensemble-slice.svg)
