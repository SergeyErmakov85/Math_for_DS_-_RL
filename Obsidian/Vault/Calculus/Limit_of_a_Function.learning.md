...
| $\enfVar{a}$ | точка, в окрестности которой исследуется функция | `variable`  |
| $\enfFun{f}$, $\enfFun{g}$, $\enfFun{h}$ | исследуемые функции                              | `function`  |
| $\enfTgt{L}$, $\enfTgt{M}$ | предельные значения                              | `target`    |
| $\enfPar{\varepsilon}$, $\enfPar{\delta}$ | положительные величины, задающие точность        | `parameter` |
| $\enfOp{\lim}$ | оператор предела                                 | `operator`  |
