...
> [!note] Задача выполнена, пути устарели
> Плагин и преамбула подключены. Вложенного хранилища `Obsidian/Vault/` больше нет: хранилищем служит корень репозитория, плагин лежит в `.obsidian/plugins/`, преамбула — в `preamble.sty`, запасная заметка — в `Obsidian/mathjax-preamble.md`. Документ сохранён как запись о принятом решении; актуальная инструкция — `docs/04_Obsidian_Guide.md`.
