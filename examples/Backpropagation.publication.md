...
|--------|----------|------|--------|
| $\enfVar{z_k}$, $\enfVar{h_k}$ | линейная комбинация и выход $k$-го слоя | `variable` | здесь |
| $\enfFun{\sigma}$ | функция активации | `function` | гл. 8.1 |
| $\enfPar{w_k}$, $\enfPar{b_k}$ | параметры $k$-го слоя | `parameter` | гл. 8.1 |
