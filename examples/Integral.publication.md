...
|--------|----------|------|--------|
| $\enfVar{x_i}$, $\enfVar{\xi_i}$ | точки разбиения и промежуточные точки | `variable` | здесь |
| $\enfFun{f}$ | подынтегральная функция | `function` | гл. 1.3 |
| $\enfPar{\Delta x_i}$, $\enfPar{\lambda}$ | длины отрезков и мелкость разбиения | `parameter` | здесь |
