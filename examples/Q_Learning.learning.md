...
|--------|----------|------|
| $\enfVar{s}$, $\enfVar{s'}$ | текущее состояние и состояние-преемник | `variable` |
| $\enfFun{a}$, $\enfFun{a'}$ | текущее и следующее действие | `function` |
| $\enfTgt{r}$ | полученная награда | `target` |
| $\enfOp{Q}(\enfVar{s}, \enfFun{a})$ | оценка ценности пары «состояние — действие» | `operator` |
| $\enfPar{\alpha} \in (0, 1]$ | скорость обучения (learning rate) | `parameter` |
| $\enfPar{\gamma} \in [0, 1)$ | коэффициент дисконтирования | `parameter` |
| $\enfPar{\varepsilon}$ | доля случайных действий при исследовании | `parameter` |
